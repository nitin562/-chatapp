...
export default function ProfileDataShow({ Icon, fields,save }) {
  ...
  return (
    <div className="flex flex-col w-full justify-center gap-4 py-2 relative border-t-[1px] border-b-[1px] border-white/20 p-1 px-4   cursor-pointer">
      ...
      {fields.map((field) => {
        return (
          <div onClick={()=>onClick(field.name,field.value)}
            key={field.name}
          ...
          >
            <div className="flex flex-col w-[90%] gap-1 ">
              <span className="text-[0.8rem] text-blue-300/50">{field.name}</span>
              {isEditing!=field.name && <p title={field.value} className="break-after-words  text-wrap text-ellipsis w-full overflow-hidden">
                {field.value||"-"}
              </p>}
              {isEditing==field.name && (field.name=="Status"?<textarea autoFocus value={updatedVal} onChange={onChange} />:<input autoFocus value={updatedVal} onChange={onChange}/>)}
              {error.name==field.raw_name&&<span className="text-red-500">{error.err}</span>}
            </div>
            <div className="w-fit flex items-center justify-end">
              {isEditing!=field.name && <CiEdit className=" text-2xl group-hover:text-blue-300/50 " />}
              {isEditing==field.name && !loading&& <CiSaveUp1 onClick={()=>{HandleSave(field.raw_name,field.value,updatedVal)}} className="text-2xl hover:text-red-400"/>}
